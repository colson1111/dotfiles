...
for i in "$BASEDIR"/*; do
    ...
    for j in "$i"/*; do
        ...
        BASEFILE="$HOME/.$FILE"
        if [ .$FILE == ".vimrc" ]; then
                echo "Updating BASEFILE for vimrc from $BASEFILE to $HOME/.vim/vimrc"
                BASEFILE="$HOME/.vim/vimrc"
        fi
        if [ -h "$BASEFILE" ]; then
        ...
        echo "Symlinking $j to $BASEFILE"
        ln -s "$j" "$BASEFILE"
    done
done

ln -s 
ln -s /Users/craig/dotfiles/nix/vim/vimrc /Users/craig/.vim/vimrc
